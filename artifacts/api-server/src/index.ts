...
function scheduleBotMessage() {
  const delay = 90_000 + Math.floor(Math.random() * 60_000); // 90–150 sn
  setTimeout(async () => {
    try {
      if (await isGuvenlikBotEnabled()) {
...
async function maybeBotReply(question: string, askerUsername: string) {
  ...
  }, 25000 + Math.random() * 25000); // 25–50 sn sonra bot yanıtı
}

function scheduleFakeConversation() {
  const delay = 75_000 + Math.floor(Math.random() * 75_000); // 75–150 sn
  setTimeout(async () => {
    try {
      ...
      if (roll < 0.35) {
        ...
        // Çift cevap: en az ~45–90 sn arayla
        const pairDelay = Math.max(45_000, Math.round(pair.delay * 2.2));
        setTimeout(() => {
          void (async () => {
...
function scheduleInfoBot() {
  const delay = 100_000 + Math.floor(Math.random() * 80_000); // 100–180 sn
  setTimeout(async () => {
    try {
