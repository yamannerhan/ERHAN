...
router.post("/chat/messages", authMiddleware, async (req, res): Promise<void> => {
  const { loadSpamSettings, effectiveSpamCooldownSeconds } = await import("../lib/spam-schedule");
  const settings = await loadSpamSettings();
  const chatLocked = settings.chatLocked;
  const spamCooldown = effectiveSpamCooldownSeconds(settings);

  if (chatLocked && req.user!.role !== "admin" && req.user!.role !== "moderator") {
    res.status(403).json({ error: "Sohbet şu an kilitli" });
    return;
  }

  // Spam: yalnızca normal üyeler — admin/mod muaf
  ...
  let safeReplyToId: number | null = replyToId ?? null;
  if (safeReplyToId) {
    const [orig] = await db.select({
      id: chatMessagesTable.id,
      userId: chatMessagesTable.userId,
    }).from(chatMessagesTable).where(eq(chatMessagesTable.id, safeReplyToId)).limit(1);
    if (!orig) {
      safeReplyToId = null;
    } else if (orig.userId === req.user!.id) {
      res.status(400).json({ error: "Kendi mesajınıza yanıt veremezsiniz" });
      return;
    }
  }
  ...
  // Kritik yol bitti — istemciye hemen dön
  ...
  void (async () => {
    ...
    if (safeReplyToId) {
      try {
        const [orig] = await db.select().from(chatMessagesTable).where(eq(chatMessagesTable.id, safeReplyToId)).limit(1);
    ...
    if (mentionNames.length > 0) {
      try {
        const { getUserNotifPrefs, prefsAllowInAppType } = await import("../lib/user-notif-prefs");
        const mentionConds = mentionNames.map((n) => sql`lower(${usersTable.username}) = ${n.toLowerCase()}`);
        const mentionedUsers = mentionConds.length > 0
          ? await db.select({ id: usersTable.id, username: usersTable.username })
              .from(usersTable)
              .where(or(...mentionConds))
          : [];
        ...
        for (const target of mentionedUsers) {
          if (target.id === req.user!.id || target.id <= 0) continue;
...
router.post("/chat/messages/:id/react", authMiddleware, async (req, res): Promise<void> => {
  ...
  if (existing) {
    ...
    // Kişi başı tek emoji: önceki tepkileri kaldır
