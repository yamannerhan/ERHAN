...
function bannerFilenameFromUrl(imageUrl: string | null | undefined): string | null {
  if (!imageUrl) return null;
  const match = imageUrl.match(/\/api\/banner-images\/([^/?#]+)/);
  return match?.[1] ? path.basename(match[1]) : null;
}

async function deleteBannerImageFile(imageUrl: string | null | undefined): Promise<void> {
  const filename = bannerFilenameFromUrl(imageUrl);
  if (!filename) return;
  try {
    await fs.promises.unlink(path.join(BANNER_IMAGES_DIR, filename));
  } catch {
    /* dosya yoksa yoksay */
  }
}
...
function settingsJson(s: typeof adminSettingsTable.$inferSelect) {
  return {
    ...
    spamNightEnabled: (s as { spamNightEnabled?: boolean }).spamNightEnabled !== false,
    spamDayEnabled: (s as { spamDayEnabled?: boolean }).spamDayEnabled === true,
...
router.get("/admin/settings", authMiddleware, requireAdmin, async (_req, res): Promise<void> => {
  const { ensureSpamScheduleColumns } = await import("../lib/spam-schedule");
...
router.patch("/admin/settings", authMiddleware, requireAdmin, async (req, res): Promise<void> => {
  const { ensureSpamScheduleColumns } = await import("../lib/spam-schedule");
  await ensureSpamScheduleColumns();
  const { chatLocked, fakeOnlineBonus, fakeOnlineMin, fakeOnlineMax, maintenanceMode, welcomeMessage, openaiApiKey, spamCooldown, spamNightEnabled, spamDayEnabled, chatAnnounceListings, chatTickerMessage, chatPinnedMessage, hiddenListingCities, botGuvenlikEnabled, botBilgiEnabled, botFakeEnabled, telegramScanIntervalMinutes, pushEnabled, pushOnNewListing, pushOnChatReply, pushSoundEnabled, pushDigestMode, pushOnUserJoin, pushSoundListingUrl, pushSoundJoinUrl, pushSoundReplyUrl, pushSoundCampaignUrl } = req.body as Record<string, unknown>;
  ...
  if (spamNightEnabled !== undefined) updates.spamNightEnabled = Boolean(spamNightEnabled);
  if (spamDayEnabled !== undefined) updates.spamDayEnabled = Boolean(spamDayEnabled);
...
router.post("/admin/banners/reorder", authMiddleware, requireAdmin, async (req, res): Promise<void> => {
  ...
  if (!Array.isArray(order) || order.length === 0) {
    res.status(400).json({ error: "Sıralama listesi gerekli" });
    return;
  }
  const ids = order.map((id) => parseInt(String(id), 10)).filter((id) => !isNaN(id));
  for (let i = 0; i < ids.length; i++) {
    await db.update(bannersTable).set({ sortOrder: i + 1 }).where(eq(bannersTable.id, ids[i]!));
  }
  res.json({ success: true });
});
