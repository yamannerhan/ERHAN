...
function RoleTitle({ role, isVip }: { role: string; isVip?: boolean }) {
  ...
  if (isVip) {
    return (
      <span
        className="inline-flex items-center gap-0.5 rounded-md px-1.5 py-[1px] text-[8px] font-black tracking-[0.12em] uppercase badge-vip"
      >
        <Crown className="w-2.5 h-2.5 fill-current" /> VIP ÜYE
      </span>
    );
  }
  return null;
...
export function ChatUserIdentity({
  ...
  else if (msg.isVip && msg.userNameAnimated) nameClass += " name-vip";
  else if (msg.isVip) nameClass += " name-vip";
  ...
  return (
    <div className={`flex flex-col gap-0.5 min-w-0 ${align === "end" ? "items-end" : "items-start"}`}>
      ...
      <span
        className={nameClass}
        style={
          role === "admin" || role === "moderator" || msg.isVip || msg.userNameAnimated
            ? undefined
            : color
              ? { color }
              : { color: "#e2e8f0" }
