...
import { BadgeCheck, ShieldOff, PauseCircle, Search, RefreshCw } from "lucide-react";
  ...
  return (
    <div className="space-y-3">
      <div className="flex flex-wrap gap-2 items-center">
        ...
        >
          <option value="all">Tümü (aktif süreç)</option>
                ...
                {canApprove && u.isVerifiedPublisher && (
                  <>
                    <button
                      ...
                      onClick={() => void act(u.id, "suspend-verification", { note: note || "Askıya alındı" })}
                    >
                    ...
                    <button
                      ...
                      onClick={() => void act(u.id, "remove-verification", { note: note || undefined })}
                    >
            ...
            {selectedId === u.id && canApprove && (
              <div className="flex flex-wrap gap-2 items-end pt-1 border-t border-white/10">
                ...
                <label className="text-[10px] flex-1 min-w-[120px] space-y-1">
                  Not
                  <input value={note} onChange={(e) => setNote(e.target.value)} className="block w-full rounded bg-black/50 border border-white/15 text-xs px-2 py-1" />
                </label>
                <button
                  type="button"
                  className="text-[10px] px-3 py-1.5 rounded bg-emerald-500 text-black font-bold"
                  onClick={() => void act(u.id, "verify-publisher", { verificationType: vType, note, syncCompanyProfile: true })}
                >
                  Onayla
                </button>
              </div>
