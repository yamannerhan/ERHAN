...
export default function Profile() {
  ...
  const [companyForm, setCompanyForm] = useState({
    companyName: "", legalName: "", description: "", website: "", phone: "", email: "",
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setCompanyLogo(data.logoPath || null);
        setCompanyVerified(!!data.isVerified);
      } catch { /* ignore */ }
    })();
  ...
  const saveCompany = async () => {
    if (!companyForm.companyName.trim()) {
      toast({ title: "Şirket adı zorunludur", variant: "destructive" });
      return;
    }
    setCompanyLoading(true);
    try {
      const res = await apiCall("/company-profiles/me", "PUT", companyForm);
      const data = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error((data as { error?: string }).error || "Kayıt başarısız");
      setCompanyLogo(data.logoPath || null);
      setCompanyVerified(!!data.isVerified);
      toast({ title: "Şirket bilgileri kaydedildi", description: "Tüm ilanlarınızda otomatik güncellenir." });
    } catch (err: unknown) {
      toast({ title: "Hata", description: err instanceof Error ? err.message : "Kayıt başarısız", variant: "destructive" });
    } finally {
  ...
  const uploadCompanyLogo = async (file: File) => {
    ...
    try {
      // Ad yoksa önce kaydet
      const putRes = await apiCall("/company-profiles/me", "PUT", companyForm);
      if (!putRes.ok) throw new Error("Şirket kaydı gerekli");
      const fd = new FormData();
      fd.append("logo", file);
      const res = await fetch("/api/company-profiles/me/logo", {
        method: "POST",
        headers: { Authorization: `Bearer ${getToken()}` },
        body: fd,
      });
      const data = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error((data as { error?: string }).error || "Logo yüklenemedi");
      setCompanyLogo(data.logoPath || null);
      toast({ title: "Şirket logosu güncellendi" });
    } catch (err: unknown) {
      toast({ title: "Hata", description: err instanceof Error ? err.message : "Logo yüklenemedi", variant: "destructive" });
    } finally {
      ...
      <div className="p-4 space-y-4">
        ...
        {isMe && (
          <section className="space-y-2.5">
            ...
            {/* Şirket Bilgileri */}
            <button onClick={() => setShowCompany(v => !v)} className="og-setting-row">
              <div className="og-setting-icon"><Building2 className="w-5 h-5" /></div>
              <div className="flex-1 min-w-0 text-left">
                <div className="og-setting-title">Şirket Bilgileri</div>
                <div className="og-setting-sub">
                  {companyForm.companyName
                    ? `${companyForm.companyName}${companyVerified ? " · Doğrulanmış" : ""}`
                    : "Firma adı ve logo — tüm ilanlarda kullanılır"}
                </div>
              </div>
              <ChevronRight className={`w-4 h-4 og-text-muted transition-transform ${showCompany ? "rotate-90" : ""}`} />
            </button>
            <AnimatePresence>
              {showCompany && (
                <motion.div
                ...
                >
                  <div className="og-setting-row flex-col items-stretch gap-3 p-4">
                    <p className="text-[11px] og-text-muted">
                      Şirket adı ve logo bir kez kaydedilir; yeni ve eski ilanlarınızda otomatik görünür. Her ilanda yeniden yüklemeniz gerekmez.
                    </p>
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex flex-col gap-1.5 flex-1 min-w-0">
                        ...
                        >
                          {companyLogoUploading ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : <Camera className="w-4 h-4 mr-2" />}
                          {companyLogo ? "Logoyu Değiştir" : "Logo Yükle"}
                        </Button>
                        {companyLogo ? (
                          <button type="button" onClick={deleteCompanyLogo} className="text-[11px] text-red-400 text-left">
                            Logoyu sil
                          </button>
                        ) : (
                          <span className="text-[10px] og-text-muted">JPG/PNG/WEBP · max 2MB · kare (önerilen 256×256)</span>
                        )}
                      </div>
                    </div>
                    <div>
                      <label className="block text-xs og-text-muted mb-1">Şirket adı *</label>
                      <Input value={companyForm.companyName} onChange={e => setCompanyForm(f => ({ ...f, companyName: e.target.value }))} placeholder="Örn: G4S Güvenlik" maxLength={120} />
                    </div>
                    <div>
                      <label className="block text-xs og-text-muted mb-1">Ticari / vergi unvanı</label>
                      <Input value={companyForm.legalName} onChange={e => setCompanyForm(f => ({ ...f, legalName: e.target.value }))} placeholder="Opsiyonel" maxLength={160} />
                    </div>
                    <div>
                      <label className="block text-xs og-text-muted mb-1">Kısa açıklama</label>
                      <Input value={companyForm.description} onChange={e => setCompanyForm(f => ({ ...f, description: e.target.value }))} placeholder="Kısa şirket tanımı" maxLength={500} />
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label className="block text-xs og-text-muted mb-1">Web sitesi</label>
                        <Input value={companyForm.website} onChange={e => setCompanyForm(f => ({ ...f, website: e.target.value }))} placeholder="https://" maxLength={200} />
                      </div>
                      <div>
                        <label className="block text-xs og-text-muted mb-1">Telefon</label>
                        <Input value={companyForm.phone} onChange={e => setCompanyForm(f => ({ ...f, phone: e.target.value }))} placeholder="0xxx" maxLength={40} />
                      </div>
                    </div>
                    <div>
                      <label className="block text-xs og-text-muted mb-1">E-posta</label>
                      <Input value={companyForm.email} onChange={e => setCompanyForm(f => ({ ...f, email: e.target.value }))} placeholder="info@firma.com" maxLength={120} />
                    </div>
                    <div>
                      <label className="block text-xs og-text-muted mb-1">Adres</label>
                      <Input value={companyForm.address} onChange={e => setCompanyForm(f => ({ ...f, address: e.target.value }))} placeholder="Açık adres" maxLength={240} />
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label className="block text-xs og-text-muted mb-1">İl</label>
                        <Input value={companyForm.city} onChange={e => setCompanyForm(f => ({ ...f, city: e.target.value }))} maxLength={80} />
                      </div>
                      <div>
                        <label className="block text-xs og-text-muted mb-1">İlçe</label>
                        <Input value={companyForm.district} onChange={e => setCompanyForm(f => ({ ...f, district: e.target.value }))} maxLength={80} />
                      </div>
                    </div>
                    {companyVerified ? (
                    ...
                    <Button onClick={saveCompany} disabled={companyLoading} className="w-full bg-gradient-to-r from-amber-400 to-amber-600 text-slate-900 font-bold">
                      {companyLoading ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : null}
